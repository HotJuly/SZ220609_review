...
​	**1.浏览器是一个多进程多线程的程序,所以js线程会有非常多的兄弟帮忙,所以他需要负责的内容相对较少**

​		 **每个管理模块其实都是一个分线程**

​	**2.浏览器只有一个宏任务队列和一个微任务队列**



#### Node事件轮询机制

事件轮询机制主要分为六个阶段(这里每一个阶段都是一个独立的宏任务队列,也就是说node有**6个宏任务队列**)：

 1.timers 计时器阶段(这是入口阶段)
...
　　准备工作
 ...
 4.poll 轮询阶段（轮询队列）(专门负责I/O操作相关内容)(这是轮询的休息区)
